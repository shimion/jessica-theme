...
class C5_Widget extends WP_Widget {
	...
	function inline_css() {
			...
			$skip = false;
			$skip = apply_filters( 'c5ab_remove_css_' . $this->_shortcode_name, $skip );
			if ($skip){
				return;
			}
	        ob_start();
	...
	function options() {
		...
		$this->_options =array(
		 ...
		 array(
		     ...
		     'id' => 'location',
		     'type' => 'select',
		     'desc' => 'Choose The Menu Location You want.',
		     'choices' => $menu_new,
		     'std' => 'main-nav',
		     'rows' => '',
		     'post_type' => '',
		 ...
		 array(
		     ...
		     'id' => 'primary_color',
		     'type' => 'background',
		     'desc' => 'Pick a the main color for the theme (default: #9acb48 ).',
		     'std' => '',
		     'rows' => '',
		     'post_type' => '',
		     'taxonomy' => '',
	...
	function update( $new_instance, $old_instance ) {
		...
		foreach ($new_instance as $key => $value) {
			if(is_array($new_instance[ $key ])){
				...
				if($key =='content'){
					$instance[ $key ] = htmlspecialchars( $new_instance[ $key ] );
				}else {
	...
	function display_setting( $args = array(), $instance= array() ) {
	      ...
	      if( ( $type == 'list-item' || $type == 'background' || $type == 'spacing' ) && $field_value!=''){
	      	unset($field_value);
	      	$field_value= array();
