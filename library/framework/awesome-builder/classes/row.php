...
class C5PB_ROW extends C5PB_BASE {
	...
	function render() {
		...
		<style>
			...
			#c5ab-section-<?php echo $this->id ?>{
				...
				<?php 
				$padding = unserialize(base64_decode($this->settings['row_padding']));
				$padding_css = $this->generate_css_spacing($padding);
				
				$margin = unserialize(base64_decode($this->settings['row_margin']));
				$margin_css = $this->generate_css_spacing($margin);
		...
		if( $device_responsive->isTablet() ){
			$GLOBALS['c5_content_width'] = 768;
		}
		
		if( $device_responsive->isMobile() && !$device_responsive->isTablet() ) {
			$GLOBALS['c5_content_width'] = 480;
		}
	...
	function generate_css_spacing($spacing) {
		
		if (ctype_digit($spacing['top'])) {
			$css  = $spacing['top'] . $spacing['unit'] . ' ';	
		}else {
			$css  = 'auto ';
		}
		
		if (ctype_digit($spacing['right'])) {
			$css  .= $spacing['right'] . $spacing['unit'] . ' ';	
		}else {
			$css  .= 'auto ';
		}
		
		if (ctype_digit($spacing['bottom'])) {
			$css  .= $spacing['bottom'] . $spacing['unit'] . ' ';	
		}else {
			$css  .= 'auto ';
		}
		
		if (ctype_digit($spacing['left'])) {
			$css  .= $spacing['left'] . $spacing['unit'] . ' ';	
		}else {
			$css  .= 'auto ';
		}
