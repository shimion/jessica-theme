...
class C5_post {
    ...
    function get_post_thumb_wide($atts) {
        ...
        $img_html = $this->get_post_image( $image_size);
    ...
    function get_menu_post() {
        ...
        if( $image_attributes ){
            ...
            $image_src_2x = $image_attributes[3];
        }
        ...
        $data .= '<style >#post-bg-'.get_the_ID().'{background-image:url("'.$image_src.'")}';
        if ($image_src_2x!='') {
			$data .= ' @media
			...
			$data .= '#post-bg-'.get_the_ID().'{background-image:url(\'' . $image_src_2x . '\');}';
			$data .= '}';
		}
        $data .='</style>';
...
//		$return .=  $img_html;
    ...
    function get_featured_media() {
        ...
        $image_attributes = c5_wp_get_attachment_image_src($post_thumbnail_id, $image_size);
        if (is_array($image_attributes)) {
            $image_attributes[2] = '';
            $css = $this->background_css('#media-'.$unique_id , $image_attributes);
    ...
    function get_post_slide($atts) {
        ...
        $image_attributes = c5_wp_get_attachment_image_src($attachment_id, $image_size);
        if (is_array($image_attributes)) {
            $css = $this->background_css('.slide-'.$ID , $image_attributes);

            $img_css = '<style>'.$css.'</style>';
            $image_src = $image_attributes[0];
        }else{
            $image_src = ot_get_option('default_cover');
    		if($image_src==''){
        ...
        $return .= $img_css;
        $return .= '<div class="c5-dark-shadow c5-content-'.  $color_mode .'" >
        	<div class="c5-main-width-wrap c5-header-data">';
...
//            'c5_weekly_shares'=>'Social Share Count in the past 7 Days',
    ...
    function background_css( $element ,  $image_data ){
        ...
        if ($image_data[2]!='') {
            $css .= 'min-height:'.$image_data[2].'px;' . "\n";
        }
        $css .= '}' . "\n";
    ...
    function get_post_image( $image_size  ){
        ...
        $image_attributes = c5_wp_get_attachment_image_src($post_thumbnail_id, $image_size);
        if (is_array($image_attributes)) {
            $css = $this->background_css('.c5-post-' . $unique_id , $image_attributes);
            $css .= '.c5-post-' . $unique_id . '{min-width: '.$image_attributes[1].'px;}';
            $img_html = '<div class="c5-post-' . $unique_id .' c5-post-thumb"><style>'.$css.'</style></div>';
        }
