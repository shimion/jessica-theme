...
class C5_build_header {
            ...
            <div  class="inner-header c5-main-width-wrap clearfix">
                <div class="c5-logo-wrap clearfix">
                    ...
                    <div class="c5-right" >
                        <?php
                        if (!is_single()) {
                            echo(do_shortcode('[c5ab_menu location="' . $c5_skindata['header_menu'] . '" responsive="on" style="default" bg_mode="light" ]'));
                        }
        ...
        if (is_category() || is_tag() || is_tax()) {
			...
			if ($c5_custom_image != '') {
                ...
                if (is_array($image_attributes)) {
                    ...
                    $image_src_2 = $image_attributes[3];
                }
            }
        } elseif (is_404()) {
                ...
                <style  >
                    ...
                    <?php if($image_src_2 != ''){ ?>
                       ...
                       only screen and (                min-resolution: 2dppx) {
                           .c5-main-header-wrap{
                               background-image: url('<?php echo $image_src_2; ?>');
                           }
                      }
                  <?php } ?>
                </style>
    ...
    function get_main_header_ad($size = '728x90', $main_ad = false) {
        ...
        $return = '';
        if (isset($c5_skindata['main_ad']) && ( $c5_skindata['main_ad'] == $size || $c5_skindata['main_ad'] == 'both' )) {
            if ($main_ad) {
                $size = '728x90';
            } else {
                $size = '300x250';
            }
            $return =  $c5_skindata['main_ad_content_' . $size] ;
        }
    ...
    function get_current_logo() {
        ...
        if ($logo_url == '' || $logo_url == 'http://C5_THEME_LOGO_URL') {
            $src = C5_URL . 'library/images/logo.png';
            $src_2x = C5_URL . 'library/images/logo-2x.png';
        } else {
            $image_data = c5_generate_image(9999, ot_get_option('logo_height'), $logo_url, false);
            $src = $image_data['src'];
            $src_2x = $image_data['src_2x'];
        }


        $home_url = apply_filters('c5_logo_url' , home_url() );

        $data = '<a class="clearfix c5-orignial-logo c5-logo"  style="margin-top:' . $top_margin . 'px" href="' . $home_url . '" rel="nofollow"><img alt="Logo" src="' . $src. '" srcset="'.$src. ' 1x, '.$src_2x.' 2x" /></a>';
